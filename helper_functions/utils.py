...
def apply_log_transform(df, spec_vars_list):
    for spec_vars in spec_vars_list:
        spec = spec_vars['spec']
        df[f'Area_mean_{spec}_log'] = np.log(df[f'Area_mean_{spec}'])
        df[f'Max_{spec}_log']       = np.log(df[f'Max_{spec}'])
    df['Release_rate_log'] = np.log(df['Release_rate'])
